...
# is idempotent.
source /usr/share/fortishield-indexer/bin/opensearch-env-from-file
...
if [[ -f bin/opensearch-users ]]; then
  ...
  if [[ -n "$ELASTIC_PASSWORD" ]]; then
    [[ -f /usr/share/fortishield-indexer/config/opensearch.keystore ]] || (run_as_other_user_if_needed opensearch-keystore create)
...
if [[ "$(id -u)" == "0" ]]; then
  ...
  if [[ -n "$TAKE_FILE_OWNERSHIP" ]]; then
    chown -R 1000:0 /usr/share/fortishield-indexer/{data,logs}
  fi
fi

run_as_other_user_if_needed /usr/share/fortishield-indexer/bin/opensearch <<<"$KEYSTORE_PASSWORD"
