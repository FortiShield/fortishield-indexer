...
%description
Fortishield indexer is a near real-time full-text search and analytics engine that
gathers security-related data into one platform. This Fortishield central component
indexes and stores alerts generated by the Fortishield server. Fortishield indexer can be
configured as a single-node or multi-node cluster, providing scalability and
high availability.
For more information, see: https://www.fortishield.com/
...
%define observability_plugin %( if [ -f %{_topdir}/etc/fortishield-indexer/opensearch-observability/observability.yml ]; then echo "1" ; else echo "0"; fi )
%define reportsscheduler_plugin %( if [ -f %{_topdir}/etc/fortishield-indexer/opensearch-reports-scheduler/reports-scheduler.yml ]; then echo "1" ; else echo "0"; fi )
...
set -- "$@" "%{config_dir}/fortishield-template.json"
...
%attr(660, %{name}, %{name}) %{config_dir}/fortishield-template.json
