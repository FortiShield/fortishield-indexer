...
show_usage() {
  ...
  echo "  * INDEXER_SRC: Path to the fortishield-indexer repository"
  ...
  echo "Example: $0 v8.10.0 ~/fortishield-indexer vulnerability-detector --upload https://indexer:9200"
}
...
generate_mappings() {
  ...
  python scripts/generator.py --strict --ref "$ECS_VERSION" \
    --include "$IN_FILES_DIR/custom/fortishield.yml" \
