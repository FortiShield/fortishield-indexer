...
# ====
function add_fortishield_tools() {
    ...
    curl -sL "${download_url}/fortishield-certs-tool.sh" -o $PATH_PLUGINS/opensearch-security/tools/fortishield-certs-tool.sh
}
...
function enable_performance_analyzer() {
    ...
    cp "distribution/packages/src/common/fortishield-indexer-performance-analyzer.service" "${TMP_DIR}"/usr/lib/systemd/system
}
...
function assemble_tar() {
    ...
    add_fortishield_tools "${version}"

    # Pack
    archive_name="fortishield-indexer-${version}"
    cd ..
...
function assemble_rpm() {
    # Copy spec
    cp "distribution/packages/src/rpm/fortishield-indexer.rpm.spec" "${TMP_DIR}"
    # Copy performance analyzer service file
    enable_performance_analyzer

    cd "${TMP_DIR}"
    local src_path="./usr/share/fortishield-indexer"
    PATH_CONF="./etc/fortishield-indexer"
    ...
    local version
    version=$(cat ./usr/share/fortishield-indexer/VERSION)
    ...
    local topdir
    local spec_file="fortishield-indexer.rpm.spec"
    topdir=$(pwd)
    rpmbuild --bb \
    ...
    cd ../../..
    package_name="fortishield-indexer-${version}-1.${SUFFIX}.${EXT}"
...
function assemble_deb() {
    ...
    # Move to the root folder, copy the package and clean.
    cd ../../..
    package_name="fortishield-indexer_${version}_${SUFFIX}.${EXT}"
...
function main() {
    ...
    echo "Assembling fortishield-indexer for $PLATFORM-$DISTRIBUTION-$ARCHITECTURE"

    ARTIFACT_BUILD_NAME=$(ls "${OUTPUT}/dist/" | grep "fortishield-indexer-min_.*$SUFFIX.*\.$EXT")
